...
def CON1O12(zonal_gdf, stats_type, rescale_min, rescale_max):
    raster_sql = f'SELECT ST_AsGDALRaster(ghanafc.rast, \'GTiff\') AS my_raster FROM "ghanafc" LIMIT 1;'
        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor:
                ...
                rescale_output = rescale.linear(
                    ...
                    input_col=f'FC_{stats_type}',
                    output_col=result_col_name,
                    start=zonal_output[f'FC_{stats_type}'].max(),
                    end=zonal_output[f'FC_{stats_type}'].min(),
                    output_min=int(rescale_min),
                    output_max=int(rescale_max)
                )
                rescale_output[result_col_name] = (
                    rescale_output[result_col_name]
