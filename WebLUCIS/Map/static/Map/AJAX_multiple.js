...
$("#road_form").submit(function (e) {
    e.preventDefault();

    // Gather form data
    let region = $('#id_region').val();
    let district = $('#id_district').val();
    let road_class = $('#road_class').val();
    let cell_size = $('#cell_size').val();
    let method = $('#method').val();
    let rescale_min = $('#rescale_min').val();
    let rescale_max = $('#rescale_max').val();
    // Perform AJAX request
    $.ajax({
        url: '/submit_road_accessibility/',  // Replace with your Django view URL
        type: 'POST',
        data: {
            'region': region,
            'district': district,
