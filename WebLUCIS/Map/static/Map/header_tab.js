// import { deckgl, zoomToRegion, getCookie } from './AJAX_multiple.js';
// import { updatemap } from './colorPalette.js';
// import { changeNodeColor } from './d3_flowchart.js';
// let tabData = {};
// export let geojsonData;
// var formname;
// // Centralized mapping of form names to corresponding columns

// export var formColumns = {
//     '/submit_road_form/': 'distance_road_rescale',
//     '/submit_popdensity_form/': 'zonal_popdens_rescale',
//     '/submit_citydist_form/': 'distance_city_rescale',
// };

// var formUrls = {
//     'Road Accessibility': '/submit_road_form/',
//     'Population Density': '/submit_popdensity_form/',
//     'Distance to City or Settlement': '/submit_citydist_form/',
// }
...
//         clearMap(); // Clear the map whenever a tab is clicked
...
//         // Update map with the corresponding geojsonData and URL
//         const tabGeojsonData = tabData[currentTabId] && tabData[currentTabId].geojsonData;
//         const tabUrl = tabData[currentTabId] && tabData[currentTabId].url;
//         console.log("Hey entra idhi");
//         console.log(tabGeojsonData);
//         clearMap();
//         if (tabGeojsonData && tabUrl) {
//             updatemap(tabGeojsonData, tabUrl);
//         } else {
...
//                     // Store geojsonData with its corresponding tabData
//                     tabData[currentTabId].geojsonData = geojsonData;
//                     tabData[currentTabId].url = url;
...
//                     var flowchart_elementid = $("#details option:selected").attr("data-id");
//                     changeNodeColor(flowchart_elementid);
...
import { deckgl, zoomToRegion, getCookie,updateSubcategory ,updateDetails,updateObjective} from './AJAX_multiple.js';
import { updatemap } from './colorPalette.js';
import { changeNodeColor } from './d3_flowchart.js';
let tabData = {};
let taburlnames = {};
...
const formUrls = {
    'Road Accessibility': '/submit_road_form/',
    'Population Density': '/submit_popdensity_form/',
   'Existing Land Use': '/submit_landusage_form/',
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function switchTab(tabId) {
        ...
        let inputs = document.querySelectorAll("#dataTable tbody tr:last-child input");
        console.log(inputs);    
        const tabContents = document.querySelectorAll('.tab-content');
        const tabButtons = document.querySelectorAll('.form-tab');
        ...
        if (storedData) {
            console.log(taburlnames[currentTabId]);
            // const decompressedData = JSON.parse(LZString.decompress(storedData));
            $.ajax({
                url: 'api/mmdas_study_area/'+taburlnames[currentTabId]+'_output', // API URL to fetch data
                ...
                success: function (response) {
                    ...
                    // Parse and decompress the response data
    ...
    //             let input = document.createElement("input");
    //             input.type = "text";
    //             input.name = inputs[i].name;  // Keeps the same name for Django to process
    //             input.className = "form-control";
    //             cell.appendChild(input);
    //         }
    //     }
    ...
    function runFunctionForStep(step) {
        ...
        if (step === 1) {
            ...
            loadFormContent(formname, function (response) {
                ...
                const runButton = step3Container.querySelector('.next-btn');
                if (runButton) {
                    let inputs = document.querySelectorAll("#dataTable tbody input");   
                    console.log(inputs);
                    inputs.forEach(input => input.addEventListener("input", addRow));
                    // document.getElementById("addRowBtn").addEventListener("click", addRow);
                    if(document.getElementById("landUsageForm")){
                    document.getElementById("landUsageForm").addEventListener("submit", function (event) {
                    ...
                    console.log(document.querySelectorAll("#dataTable tbody input"));
                    if(document.getElementById("toggleButton")){
                    document.getElementById("toggleButton").addEventListener("click", function () {
                        var mainTable = document.getElementById("dataTable");
                        var alternateTable = document.getElementById("alternateTable");
                
                        if (mainTable.classList.contains("hidden")) {
                            mainTable.classList.remove("hidden");
                            alternateTable.classList.add("hidden");
                        } else {
                            mainTable.classList.add("hidden");
                            alternateTable.classList.remove("hidden");
                        }
                    });}
                    runButton.addEventListener('click', function () {
                        handleRunButtonClick();
        ...
        if (url) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    const compressedData = LZString.compress(JSON.stringify(geojsonData));
                    sessionStorage.setItem(currentTabId, compressedData);
                    console.log(geojsonData.features.at(0).properties.district_c.toLowerCase());
                    taburlnames[currentTabId] = geojsonData.features.at(0).properties.district_c.toLowerCase();
